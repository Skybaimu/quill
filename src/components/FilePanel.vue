    ...
    <div class="file-list" @click="scopeOpen = false">
      ...
      >
        <div class="file-item-top">
          <div class="file-name">
            <span v-if="isPasswordFile(file.id) && !isGlobalUnlocked()" class="file-star" style="margin-right: 2px;">
            ...
            <span v-else-if="isPasswordFile(file.id) && isGlobalUnlocked()" class="file-star" style="margin-right: 2px;">
...
function handleFileClick(file) {
  if (isPasswordFile(file.id)) {
    if (!isGlobalUnlocked()) {
      store.lockFileId = '__global__'
      store.lockMode = 'unlock'
      store.lockCallback = () => {
        unlockGlobal() // <--- Fix: Mark as unlocked globally
        selectFile(file.id)
      }
